import React, { useState, useEffect } from "react";
import { ChevronRight, ChevronLeft } from "lucide-react";
import { db } from "../firebaseConfig";
import { motion } from "framer-motion";
import { FlickeringGrid } from "@/components/ui/flickering-grid";


import {
  collection,
  getDocs,
  addDoc,
  deleteDoc,
  doc,
  onSnapshot,
  query,
  where
} from "firebase/firestore";

export default function ExpandingSidePanel() {
  const [isExpanded, setIsExpanded] = useState(false);
  const [task, setTask] = useState("");
  const [tasks, setTasks] = useState([]);

  const toggleExpansion = () => {
    setIsExpanded(!isExpanded);
  };

  // Fetch tasks in real-time
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "tasks"), (snapshot) => {
      const tasksList = snapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      setTasks(tasksList);
    });

    return () => unsubscribe(); // Cleanup on unmount
  }, []);

  // Add Task
  const addTask = async (e) => {
    e.preventDefault();
    if (task.trim() === "") {
      alert("Task cannot be empty!");
      return;
    }
  
    try {
      // Check if a task with the same name already exists
      const tasksSnapshot = await getDocs(collection(db, "tasks"));
      const taskExists = tasksSnapshot.docs.some(
        (doc) => doc.data().task.toLowerCase() === task.trim().toLowerCase()
      );
  
      if (taskExists) {
        alert("Task already exists!");
        return;
      }
  
      // Add the task if it doesn't exist
      await addDoc(collection(db, "tasks"), {
        task: task,
        timestamp: new Date(),
      });
      setTask(""); // Clear input field
    } catch (error) {
      console.error("Error adding task: ", error);
    }
  };
  

  // Remove Task
  const removeTask = async (taskId) => {
    try {
      await deleteDoc(doc(db, "tasks", taskId));
      
    } catch (error) {
      console.error("Error removing task: ", error);
    }
  };

  const [movieGenre, setMovieGenre] = useState("");
    const [movieGenreArr, setMovieGenreArr] = useState([]);
    const [moviesbyGenre, setMoviesbyGenre] = useState({});
    
    // State to manage input and suggestions for each genre
    const [genreInputs, setGenreInputs] = useState({});
    const [genreSuggestions, setGenreSuggestions] = useState({});
  
    useEffect(() => {
      const fetchGenresAndMovies = async () => {
        try {
          const genresSnapshot = await getDocs(collection(db, "genres"));
          const moviesSnapshot = await getDocs(collection(db, "movies"));
  
          const genres = genresSnapshot.docs.map((doc) => ({
            id: doc.id,
            ...doc.data(),
          }));
  
          const movies = moviesSnapshot.docs.map((doc) => ({
            id: doc.id,
            ...doc.data(),
          }));
  
          const genreMap = genres.reduce((acc, genre) => {
            acc[genre.name] = [];
            return acc;
          }, {});
  
          movies.forEach((movie) => {
            const genre = genres.find((g) => g.id === movie.genre_id);
            if (genre) genreMap[genre.name].push(movie.title);
          });
  
          setMovieGenreArr(Object.keys(genreMap));
          setMoviesbyGenre(genreMap);
        } catch (err) {
          console.error("Error fetching genres and movies:", err);
        }
      };
  
      fetchGenresAndMovies();
    }, []);
  
    const addGenre = async (e) => {
      e.preventDefault();
      
      if (!movieGenre.trim()) {
        alert("Genre cannot be empty!");
        return;
      }
    
      try {
        // Query Firestore to check for an existing genre with the same name
        const genresRef = collection(db, "genres");
        const querySnapshot = await getDocs(genresRef);
    
        const genreExists = querySnapshot.docs.some(
          (doc) => doc.data().name.toLowerCase() === movieGenre.trim().toLowerCase()
        );
    
        if (genreExists) {
          alert("Genre already exists!");
          return;
        }
    
        // Add the new genre to Firestore if no duplicate is found
        await addDoc(genresRef, { name: movieGenre });
    
        // Update local state with the new genre
        setMovieGenreArr([...movieGenreArr, movieGenre]);
        setMoviesbyGenre({ ...moviesbyGenre, [movieGenre]: [] });
        setMovieGenre(""); // Clear the input field
      } catch (err) {
        console.error("Error adding genre:", err);
      }
    };
    
  
    const removeGenre = async (genreToRemove) => {
      try {
        // Find the genre document
        const genreQuery = query(collection(db, "genres"), where("name", "==", genreToRemove));
        const genreSnapshot = await getDocs(genreQuery);
        
        if (genreSnapshot.empty) {
          throw new Error("Genre not found");
        }
  
        // Get the genre ID
        const genreId = genreSnapshot.docs[0].id;
  
        // Delete all movies associated with this genre
        const moviesQuery = query(collection(db, "movies"), where("genre_id", "==", genreId));
        const moviesSnapshot = await getDocs(moviesQuery);
  
        // Delete all associated movies
        const deleteMoviePromises = moviesSnapshot.docs.map((movieDoc) => 
          deleteDoc(doc(db, "movies", movieDoc.id))
        );
        await Promise.all(deleteMoviePromises);
  
        // Delete the genre itself
        await deleteDoc(doc(db, "genres", genreId));
  
        // Update local state
        setMovieGenreArr(movieGenreArr.filter((genre) => genre !== genreToRemove));
        const updatedMoviesbyGenre = {...moviesbyGenre};
        delete updatedMoviesbyGenre[genreToRemove];
        setMoviesbyGenre(updatedMoviesbyGenre);
  
        // Clear any related inputs or suggestions
        setGenreInputs(prev => {
          const updated = {...prev};
          delete updated[genreToRemove];
          return updated;
        });
        setGenreSuggestions(prev => {
          const updated = {...prev};
          delete updated[genreToRemove];
          return updated;
        });
      } catch (err) {
        console.error("Error removing genre:", err);
      }
    };
  
    const handleSearchChange = async (genre, searchTerm) => {
      // Update genre-specific input
      setGenreInputs(prev => ({
        ...prev,
        [genre]: searchTerm
      }));
  
      if (searchTerm.length > 2) {
        try {
          const response = await fetch(`https://www.omdbapi.com/?s=${searchTerm}&apikey=${import.meta.env.VITE_OMDB_KEY}`);
          const data = await response.json();
          if (data.Search) {
            // Limit suggestions and remove duplicates
            const uniqueSuggestions = Array.from(
              new Set(data.Search.map(item => item.Title))
            ).slice(0, 4);
            
            // Update suggestions for specific genre
            setGenreSuggestions(prev => ({
              ...prev,
              [genre]: uniqueSuggestions
            }));
          } else {
            // Clear suggestions for specific genre
            setGenreSuggestions(prev => ({
              ...prev,
              [genre]: []
            }));
          }
        } catch (error) {
          console.error("Error fetching data from OMDb API:", error);
          setGenreSuggestions(prev => ({
            ...prev,
            [genre]: []
          }));
        }
      } else {
        // Clear suggestions for specific genre
        setGenreSuggestions(prev => ({
          ...prev,
          [genre]: []
        }));
      }
    };
  
    const addMovie = async (e, genre) => {
      e.preventDefault();
      const movieTitle = genreInputs[genre]?.trim() || '';
      
      if (!movieTitle) {
        alert("Movie title cannot be empty!");
        return;
      }
    
      try {
        // Query Firestore for the specified genre
        const genreQuery = query(collection(db, "genres"), where("name", "==", genre));
        const genreSnapshot = await getDocs(genreQuery);
        const genreDoc = genreSnapshot.docs[0];
    
        if (genreDoc) {
          // Check if the movie already exists in Firestore for the specific genre (case-insensitive)
          const movieQuery = query(
            collection(db, "movies"),
            where("genre_id", "==", genreDoc.id),
            where("title", "==", movieTitle.toLowerCase()) // Ensure title is lowercase in the query
          );
          const movieSnapshot = await getDocs(movieQuery);
    
          if (!movieSnapshot.empty) {
            alert("This movie already exists in the selected genre!");
            return;
          }
    
          // Add the movie if no duplicate exists
          await addDoc(collection(db, "movies"), {
            genre_id: genreDoc.id,
            title: movieTitle.toLowerCase(), // Store the movie title in lowercase
          });
    
          // Update the local state to reflect the addition
          setMoviesbyGenre({
            ...moviesbyGenre,
            [genre]: [...moviesbyGenre[genre], movieTitle],
          });
    
          // Clear input and suggestions for this genre
          setGenreInputs((prev) => ({
            ...prev,
            [genre]: '',
          }));
          setGenreSuggestions((prev) => ({
            ...prev,
            [genre]: [],
          }));
        }
      } catch (err) {
        console.error("Error adding movie:", err);
      }
    };
    
    
  
    const removeMovie = async (genre, movieTitle) => {
      try {
        const genreQuery = query(collection(db, "genres"), where("name", "==", genre));
        const genreSnapshot = await getDocs(genreQuery);
        const genreDoc = genreSnapshot.docs[0];
  
        if (genreDoc) {
          const movieQuery = query(
            collection(db, "movies"),
            where("genre_id", "==", genreDoc.id),
            where("title", "==", movieTitle)
          );
  
          const movieSnapshot = await getDocs(movieQuery);
          const movieDoc = movieSnapshot.docs[0];
  
          if (movieDoc) {
            await deleteDoc(doc(db, "movies", movieDoc.id));
            setMoviesbyGenre({
              ...moviesbyGenre,
              [genre]: moviesbyGenre[genre].filter((m) => m !== movieTitle),
            });
          }
        }
      } catch (err) {
        console.error("Error removing movie:", err);
      }
    };

  return (
    <div className="flex h-screen w-full">
  <aside
    className={`fixed left-0 top-0 z-50 h-full transition-all duration-300 ease-in-out bg-white border-r border-slate-100 shadow-xl ${
      isExpanded ? "w-84" : "w-10"
    }`}
  >
    <div className="h-full flex flex-col relative">
      {/* Sidebar Header */}
      <div className="h-16 flex items-center justify-between px-4">
        <div 
          className={`flex items-center space-x-3 transition-opacity duration-300 ${
            isExpanded ? "opacity-100" : "opacity-0"
          }`}
        >
          <span className="text-xl font-bold text-indigo-600">Daily Workflow</span>
        </div>
      </div>

      {/* Add Task Form */}
      {isExpanded && (
        <div className="px-4 py-4">
          <form onSubmit={addTask} className="space-y-2">
            <div>
              <label htmlFor="task" className="block text-sm font-medium text-slate-600 mb-2">
                Create New Task
              </label>
              <div className="flex space-x-2">
                <input
                  type="text"
                  id="task"
                  value={task}
                  onChange={(e) => setTask(e.target.value)}
                  placeholder="What needs to be done?"
                  className="flex-grow px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
                <button
                  type="submit"
                  className="bg-indigo-600 text-white px-3 py-1 rounded-lg hover:bg-indigo-700 transition-colors"
                >
                  Add
                </button>
              </div>
            </div>
          </form>
        </div>
      )}

      {/* Task List */}
      {isExpanded && (
        <div className="px-4 py-2 flex-grow overflow-y-auto">
          <h2 className="text-lg font-semibold mb-4 text-slate-700">Active Tasks</h2>
          {tasks.length === 0 ? (
            <p className="text-slate-400 text-center">No tasks yet</p>
          ) : (
            <ul className="space-y-3">
              {tasks.map((task) => (
                <motion.li
                  key={task.id}
                  initial={{ opacity: 0, x: -20 }}
                  animate={{ opacity: 1, x: 0 }}
                  className="flex justify-between items-center bg-slate-50 px-4 py-3 rounded-lg border border-slate-100"
                >
                  <span className="text-slate-800 flex-grow truncate mr-2">{task.task}</span>
                  <button
                    onClick={() => removeTask(task.id)}
                    className="text-red-500 hover:text-red-600 bg-red-50 px-2 py-1 rounded-md"
                  >
                    Remove
                  </button>
                </motion.li>
              ))}
            </ul>
          )}
        </div>
      )}

      {/* Sidebar Footer */}
      {isExpanded && (
        <div className="px-4 py-4 border-t border-slate-100">
          <div className="flex items-center justify-between text-slate-500">
            <span>Total Tasks: {tasks.length}</span>
          </div>
        </div>
      )}

      {/* Toggle Button */}
      <button
        onClick={toggleExpansion}
        className="absolute top-1/2 transform -translate-y-1/2 -right-4 w-8 h-12 bg-white border border-slate-100 shadow-md flex items-center justify-center hover:bg-slate-50 transition-colors rounded-r-lg"
        aria-label={isExpanded ? "Collapse sidebar" : "Expand sidebar"}
      >
        {isExpanded ? (
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
        ) : (
          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
          </svg>
        )}
      </button>
    </div>
  </aside>



      {/* Main Content */}
      <div className="min-h-screen w-full flex flex-col items-center justify-start bg-gradient-to-br from-slate-50 to-white px-4 py-10 font-inter">

  <div className="max-w-5xl mx-auto">
    <motion.header 
      initial={{ opacity: 0, y: -30 }}
      animate={{ opacity: 1, y: 0 }}
      className="text-center mb-16"
    >
      <h1 className="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600 mb-4">
        Cinematic Vault
      </h1>
      <p className="text-xl text-slate-500 max-w-2xl mx-auto">
        Curate and organize your movie collection with precision and style
      </p>
    </motion.header>

    <motion.form
      onSubmit={addGenre}
      initial={{ opacity: 0, scale: 0.95 }}
      animate={{ opacity: 1, scale: 1 }}
      className="max-w-2xl mx-auto mb-16 bg-white shadow-2xl rounded-2xl p-8 border border-slate-100"
    >
      <div className="flex space-x-4">
        <input
          type="text"
          value={movieGenre}
          onChange={(e) => setMovieGenre(e.target.value)}
          placeholder="Enter a new movie genre"
          className="flex-grow px-5 py-4 text-lg border-2 border-slate-200 rounded-xl focus:outline-none focus:border-indigo-500 transition-all duration-300"
        />
        <motion.button
          type="submit"
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-6 rounded-xl hover:shadow-xl transition-all duration-300"
        >
          Add
        </motion.button>
      </div>
    </motion.form>

    {movieGenreArr.length === 0 && (
      <motion.div 
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="text-center text-slate-400 text-lg"
      >
        Begin your movie collection journey by adding a genre
      </motion.div>
    )}

    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="grid md:grid-cols-2 lg:grid-cols-3 gap-8"
    >
      {movieGenreArr.map((genre, index) => (
        <motion.div
          key={genre}
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: index * 0.1 }}
          className="bg-white border border-slate-100 rounded-2xl shadow-xl p-6 hover:shadow-2xl transition-all duration-300"
        >
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600">
              {genre}
            </h2>
            <motion.button
              onClick={() => removeGenre(genre)}
              whileHover={{ scale: 1.2 }}
              whileTap={{ scale: 0.9 }}
              className="text-red-500 hover:text-red-600"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              </svg>
            </motion.button>
          </div>

          <form onSubmit={(e) => addMovie(e, genre)} className="mb-6">
            <div className="flex space-x-2">
              <input
                type="text"
                value={genreInputs[genre] || ''}
                onChange={(e) => handleSearchChange(genre, e.target.value)}
                placeholder={`Add movie to ${genre}`}
                className="flex-grow px-4 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-500 transition-all"
              />
              <motion.button
                type="submit"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="bg-indigo-500 text-white px-4 py-2 rounded-lg hover:bg-indigo-600 transition-colors"
              >
                Add
              </motion.button>
            </div>
          </form>
          <div>
      {/* Genre Suggestions Dropdown */}
      {genreSuggestions[genre] && genreSuggestions[genre].length > 0 && (
        <div className="mt-2 border border-slate-200 rounded-lg bg-white shadow-lg max-h-60 overflow-y-auto">
          <ul>
            {genreSuggestions[genre].map((suggestion, index) => (
              <li
                key={index}
                onClick={() => {
                  setGenreInputs((prev) => ({
                    ...prev,
                    [genre]: suggestion,
                  }));
                  setGenreSuggestions((prev) => ({
                    ...prev,
                    [genre]: [], // Clear suggestions after selecting one
                  }));
                }}
                className="px-4 py-2 cursor-pointer hover:bg-indigo-100"
              >
                {suggestion}
              </li>
            ))}
          </ul>
        </div>
      )}

      {/* Movie List for the selected Genre */}
      <ul className="space-y-3">
        {moviesbyGenre[genre]?.map((m, index) => (
          <motion.li
            key={index}
            initial={{ opacity: 0, x: -10 }}
            animate={{ opacity: 1, x: 0 }}
            className="flex justify-between items-center bg-slate-50 px-4 py-3 rounded-lg"
          >
            <span className="text-slate-700 truncate">{m}</span>
            <motion.button
              onClick={() => removeMovie(genre, m)}
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.95 }}
              className="ml-2 bg-red-500 text-white px-3 py-1 rounded-md text-sm hover:bg-red-600"
            >
              Remove
            </motion.button>
          </motion.li>
        ))}
      </ul>
    </div>
        </motion.div>
      ))}
    </motion.div>
  </div>
</div>
    </div>
  );
}
